...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (ngindb.Database, error) {
	if ctx.datadir == "" {
		return ngindb.NewMemDatabase()
	}
	return ngindb.NewLDBDatabase(filepath.Join(ctx.datadir, name), cache, handles)
}
