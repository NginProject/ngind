...
import (
	...
	"github.com/NginProject/ngind/internal/jsre"
	"github.com/NginProject/ngind/internal/web3ext"
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var ngin = web3.ngin; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = ngin.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines ngin.getBalance(ngin.coinb<tab><tab>
	start := 0
...
// Welcome show summary of current ngind instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic ngind metadata
	fmt.Fprintf(c.printer, "Welcome to the ngind JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + ngin.coinbase);
		console.log("at block: " + ngin.blockNumber + " (" + new Date(1000 * ngin.getBlock(ngin.blockNumber).timestamp) + ")");
