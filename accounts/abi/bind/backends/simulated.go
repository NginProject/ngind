...
type SimulatedBackend struct {
	database   ngindb.Database  // In memory database to store our testing data
...
func NewSimulatedBackend(accounts ...core.GenesisAccount) *SimulatedBackend {
	database, _ := ngindb.NewMemDatabase()
	core.WriteGenesisBlockForTesting(database, accounts...)
	blockchain, _ := core.NewBlockChain(database, core.DefaultConfigTestnet.ChainConfig, new(core.FakePow), new(event.TypeMux))
...
func (b *SimulatedBackend) Rollback() {
	blocks, _ := core.GenerateChain(core.DefaultConfigTestnet.ChainConfig, b.blockchain.CurrentBlock(), b.database, 1, func(int, *core.BlockGen) {})
...
func (b *SimulatedBackend) ContractCall(contract common.Address, data []byte, pending bool) ([]byte, error) {
	...
	vmenv := core.NewEnv(statedb, core.DefaultConfigTestnet.ChainConfig, b.blockchain, msg, block.Header())
...
func (b *SimulatedBackend) SendTransaction(tx *types.Transaction) error {
	blocks, _ := core.GenerateChain(core.DefaultConfigTestnet.ChainConfig, b.blockchain.CurrentBlock(), b.database, 1, func(number int, block *core.BlockGen) {
