...
const (
	...
	subscribeMethod        = "ngin_subscribe"
	unsubscribeMethod      = "ngin_unsubscribe"
	notificationMethod     = "ngin_subscription"
)
...
func parseRequest(incomingMsg json.RawMessage) ([]rpcRequest, bool, RPCError) {
	...
	if in.Method == subscribeMethod {
		...
		if len(in.Payload) > 0 {
			...
			// all subscriptions are made on the ngin service
			reqs[0].service, reqs[0].method = "ngin", subscribeMethod[0]
	...
	for i, r := range in {
		...
		if r.Method == subscribeMethod {
			...
			if len(r.Payload) > 0 {
				...
				// all subscriptions are made on the ngin service
				requests[i].service, requests[i].method = "ngin", subscribeMethod[0]
