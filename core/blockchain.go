...
// Processor.
func NewBlockChain(chainDb ngindb.Database, config *ChainConfig, pow pow.PoW, mux *event.TypeMux) (*BlockChain, error) {
...
func NewBlockChainDryrun(chainDb ngindb.Database, config *ChainConfig, pow pow.PoW, mux *event.TypeMux) (*BlockChain, error) {
	bodyCache, _ := lru.New(bodyCacheLimit)
	bodyRLPCache, _ := lru.New(bodyCacheLimit)
	blockCache, _ := lru.New(blockCacheLimit)
	futureBlocks, _ := lru.New(maxFutureBlocks)
...
func (bc *BlockChain) blockIsGenesis(b *types.Block) bool {
	...
	ht, _ := DefaultConfigTestnet.Genesis.Header()
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	if bc.atxi != nil && bc.atxi.AutoMode {
		ldb, ok := bc.atxi.Db.(*ngindb.LDBDatabase)
		if !ok {
		...
		pre := ngindb.NewBytesPrefix(txAddressIndexPrefix)
...
func (bc *BlockChain) WriteBlockAddrTxIndexesBatch(indexDb ngindb.Database, startBlockN, stopBlockN, stepN uint64) (txsCount int, err error) {
