...
func NewHeaderChain(chainDb ngindb.Database, config *ChainConfig, mux *event.TypeMux, getValidator getHeaderValidatorFn, procInterrupt func() bool) (*HeaderChain, error) {
	...
	if config == DefaultConfigTestnet.ChainConfig {
		gen = DefaultConfigTestnet.Genesis
