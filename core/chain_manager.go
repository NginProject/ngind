...
func GenerateChain(config *ChainConfig, parent *types.Block, db ngindb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
...
// header only chain.
func newCanonical(config *ChainConfig, n int, full bool) (ngindb.Database, *BlockChain, error) {
	// Create the new chain database
	db, err := ngindb.NewMemDatabase()
	if err != nil {
		return nil, nil, err
	}
	...
	genesis, err := WriteGenesisBlock(db, DefaultConfigTestnet.Genesis)
	if err != nil {
		return nil, nil, err
	}
...
func makeHeaderChain(config *ChainConfig, parent *types.Header, n int, db ngindb.Database, seed int) []*types.Header {
...
func makeBlockChain(config *ChainConfig, parent *types.Block, n int, db ngindb.Database, seed int) []*types.Block {
