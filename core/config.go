...
type SufficientChainConfig struct {
	...
	Network         int              `json:"network"`   // ngin.NetworkId (mainnet=1, testnet=2)
	Consensus       string           `json:"consensus"` // pow type (M00N OR M00N-test => Will be others in future)
...
func parseExternalChainConfig(mainConfigFile string, open func(string) (io.ReadCloser, error)) (*SufficientChainConfig, error) {
	...
	// Make 'M00N' default (backwards compatibility)
	if config.Consensus == "" {
...
func WriteGenesisBlock(chainDb ngindb.Database, genesis *GenesisDump) (*types.Block, error) {
...
func WriteGenesisBlockForTesting(db ngindb.Database, accounts ...GenesisAccount) *types.Block {
	dump := GenesisDump{
		GasLimit:   "0x47E7C4",
		Difficulty: "0x020000",
...
func MakeGenesisDump(chaindb ngindb.Database) (*GenesisDump, error) {
