...
var (
	...
	monitorCommand = cli.Command{
		Action: monitor,
		Name:   "monitor",
		Usage:  `Ngind Monitor: node metrics monitoring and visualization`,
		Description: `
	The ngind monitor is a tool to collect and visualize various internal metrics
...
func monitor(ctx *cli.Context) error {
	...
	if len(monitored) == 0 {
		...
		if len(list) > 0 {
			...
			log.Fatal("No metrics collected by ngind (--metrics).")
		}
	}
...
// retrieveMetrics contacts the attached ngind node and retrieves the entire set
...
// eg.
// $ ngind monitor [--attach=api-endpoint] metric1 metric2 ... metricN
//
