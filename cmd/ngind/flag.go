...
import (
	...
	"github.com/NginProject/ngind/whisper"
...
// chainIsTestnet allows either
// '--testnet' (legacy), or
// '--chain=testnet|testnet'
func chainIsTestnet(ctx *cli.Context) bool {
	if ctx.GlobalBool(aliasableName(TestNetFlag.Name, ctx)) {
		return true
	}
	if _, ok := core.ChainIdentitiesTestnet[core.GetCacheChainIdentity()]; ok {
		return ok
	}
	if _, ok := core.ChainIdentitiesTestnet[ctx.GlobalString(aliasableName(ChainIdentityFlag.Name, ctx))]; ok {
		return ok
	}
	if c := core.GetCacheChainConfig(); c != nil {
		if _, ok := core.ChainIdentitiesTestnet[c.Identity]; ok {
			return ok
		}
	}
	return false
...
// It returns one of valid strings: ["mainnet", "testnet", or --chain="flaggedCustom"]
func mustMakeChainIdentity(ctx *cli.Context) (identity string) {
	...
	if chainIsTestnet(ctx) {
		// This makes '--testnet', '--chain=testnet', and '--chain=testnet' all use the same /testnet subdirectory, if --chain isn't specified
		identity = core.DefaultConfigTestnet.Identity
		return identity
	}
	// If --chain is in use.
	if chainFlagVal := ctx.GlobalString(aliasableName(ChainIdentityFlag.Name, ctx)); chainFlagVal != "" {
		...
		if fstat, ferr := os.Stat(filepath.Clean(chainFlagVal)); ferr == nil && !fstat.IsDir() {
			...
			if configurationError == nil {
				...
				if core.ChainIdentitiesTestnet[c.Identity] || core.ChainIdentitiesMain[c.Identity] {
					if e := ctx.Set(aliasableName(ChainIdentityFlag.Name, ctx), c.Identity); e != nil {
						glog.Fatalf("Could not set global context chain identity to testnet, error: %v", e)
					}
				}
				identity = c.Identity
				return identity
...
func mustMakeChainConfigNameDefaulty(ctx *cli.Context) string {
	if chainIsTestnet(ctx) {
		return core.DefaultConfigTestnet.Name
	}
...
func MakeBootstrapNodesFromContext(ctx *cli.Context) []*discover.Node {
	...
	if !ctx.GlobalIsSet(aliasableName(BootnodesFlag.Name, ctx)) {
		...
		// --testnet/--chain=testnet flag overrides --config flag
		if chainIsTestnet(ctx) {
			return core.DefaultConfigTestnet.ParsedBootstrap
		}
...
// for ngind and returns half of the allowance to assign to the database.
...
// MakeCoinbase retrieves the coinbase either from the directly specified
// command line flags or from the keystore if CLI indexed.
func MakeCoinbase(accman *accounts.Manager, ctx *cli.Context) common.Address {
	accounts := accman.Accounts()
	if !ctx.GlobalIsSet(aliasableName(CoinbaseFlag.Name, ctx)) && len(accounts) == 0 {
		glog.V(logger.Warn).Warnf("No coinbase set and no accounts found as default")
		glog.D(logger.Warn).Warnf("No coinbase set and no accounts found as default")
		return common.Address{}
	}
	coinbase := ctx.GlobalString(aliasableName(CoinbaseFlag.Name, ctx))
	if coinbase == "" {
		return common.Address{}
	}
	// If the specified coinbase is a valid address, return it
	account, err := MakeAddress(accman, coinbase)
	if err != nil {
		log.Fatalf("Option %q: %v", aliasableName(CoinbaseFlag.Name, ctx), err)
	}
...
func makeNodeName(version string, ctx *cli.Context) string {
	name := fmt.Sprintf("ngind/%s/%s/%s", version, runtime.GOOS, runtime.Version())
...
func MakeSystemNode(version string, ctx *cli.Context) *node.Node {
	...
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		return ngin.New(ctx, ethConf)
	}); err != nil {
		glog.Fatalf("%v: failed to register the Ngin service: ", ErrStackFail, err)
	}
	if shhEnable {
	...
	if ctx.GlobalBool(Unused1.Name) {
		glog.V(logger.Warn).Warnln(fmt.Sprintf("ngind started with --%s flag, which is unused and can be omitted", Unused1.Name))
	}
...
func shouldAttemptDirMigration(ctx *cli.Context) bool {
	if !ctx.GlobalIsSet(aliasableName(DataDirFlag.Name, ctx)) {
		if chainVal := mustMakeChainIdentity(ctx); core.ChainIdentitiesMain[chainVal] || core.ChainIdentitiesTestnet[chainVal] {
			return true
		}
	}
	return false
...
func mustMakeEthConf(ctx *cli.Context, sconf *core.SufficientChainConfig) *ngin.Config {
	...
	// Override any default configs in dev mode
	if ctx.GlobalBool(aliasableName(DevModeFlag.Name, ctx)) {
		// Override the Ngin protocol configs
...
func mustMakeSufficientChainConfig(ctx *cli.Context) *core.SufficientChainConfig {
	...
	if core.ChainIdentitiesMain[chainIdentity] || core.ChainIdentitiesTestnet[chainIdentity] {
		...
		if chainIsTestnet(ctx) {
			config.Network = 2
			config.Genesis = core.DefaultConfigTestnet.Genesis
	...
	if _, de := os.Stat(defaultChainConfigPath); de != nil && os.IsNotExist(de) {
		...
		$ ngind --chain testnet dump-chain-config %v/chain.json
		$ sed -i.bak s/testnet/%v/ %v/chain.json
...
func logChainConfiguration(ctx *cli.Context, config *core.SufficientChainConfig) {
	chainIdentity := mustMakeChainIdentity(ctx)
	chainIsCustom := !(core.ChainIdentitiesMain[chainIdentity] || core.ChainIdentitiesTestnet[chainIdentity])
	...
	glog.V(logger.Info).Infof("Starting ngind %s", ctx.App.Version)
	glog.D(logger.Warn).Infof("ngind version: %s", logger.ColorGreen(ctx.App.Version))

	glog.V(logger.Info).Infof("ngind is configured to use blockchain: %v", config.Name)
...
func MustMakeChainConfigFromDefaults(ctx *cli.Context) *core.ChainConfig {
	...
	if chainIsTestnet(ctx) {
		c = core.DefaultConfigTestnet.ChainConfig
	}
	return c
}

// MakeChainDatabase open an LevelDB using the flags passed to the client and will hard crash if it fails.
func MakeChainDatabase(ctx *cli.Context) ngindb.Database {
	var (
		chaindir = MustMakeChainDataDir(ctx)
	...
	chainDb, err := ngindb.NewLDBDatabase(filepath.Join(chaindir, "chaindata"), cache, handles)
	if err != nil {
...
func MakeIndexDatabase(ctx *cli.Context) ngindb.Database {
	var (
		chaindir = MustMakeChainDataDir(ctx)
		cache    = ctx.GlobalInt(aliasableName(CacheFlag.Name, ctx))
		handles  = MakeDatabaseHandles()
	...
	indexesDb, err := ngindb.NewLDBDatabase(filepath.Join(chaindir, "indexes"), cache, handles)
	if err != nil {
		glog.Fatal("Could not open database: ", err)
	}
	return indexesDb
...
func MakeChain(ctx *cli.Context) (chain *core.BlockChain, chainDb ngindb.Database) {
	var err error
