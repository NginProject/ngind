...
func formatEthConfigPretty(ngConfig *ngin.Config) (s []string) {
	ss := []printable{}

	// NetworkID
	ss = append(ss, printable{0, "Network", ngConfig.NetworkId})
	// FastSync?
	ss = append(ss, printable{0, "Fast sync", ngConfig.FastSync})
	// BlockChainVersion
	ss = append(ss, printable{0, "Blockchain version", ngConfig.BlockChainVersion})
	// DatabaseCache
	ss = append(ss, printable{0, "Database cache (MB)", ngConfig.DatabaseCache})
	// DatabaseHandles
	ss = append(ss, printable{0, "Database file handles", ngConfig.DatabaseHandles})
	// NatSpec?
	ss = append(ss, printable{0, "NAT spec", ngConfig.NatSpec})
	// PowTest?
	ss = append(ss, printable{0, "Pow test", ngConfig.PowTest})
	// PowShared?
	ss = append(ss, printable{0, "Pow shared", ngConfig.PowShared})
	// SolcPath
	ss = append(ss, printable{0, "Solc path", ngConfig.SolcPath})

	// Account Manager
	lenAccts := len(ngConfig.AccountManager.Accounts())
	...
	//		ScryptP
	// Coinbase (if set)
	ss = append(ss, printable{0, "Coinbase", ngConfig.Coinbase.Hex()})
	// GasPrice
	ss = append(ss, printable{0, "Gas price", ngConfig.GasPrice})
	ss = append(ss, printable{0, "GPO min gas price", ngConfig.GpoMinGasPrice})
	ss = append(ss, printable{0, "GPO max gas price", ngConfig.GpoMaxGasPrice})
	// MinerThreads
	ss = append(ss, printable{0, "Miner threads", ngConfig.MinerThreads})
...
func status(ctx *cli.Context) error {
	...
	printme := []struct {
		...
		{"Ngin configuration", formatEthConfigPretty(ethConf)},
	...
	if !shouldUseExisting {
		glog.D(logger.Warn).Warnln("ngind has not been initialized; no database information available yet.")
		return nil
	}
...
func dumpChainConfig(ctx *cli.Context) error {
	...
	if !(core.ChainIdentitiesMain[chainIdentity] || core.ChainIdentitiesTestnet[chainIdentity]) {
		glog.Fatal("Dump config should only be used with default chain configurations (mainnet or testnet).")
	}
	...
	// Implicitly favor Testnet because it is a smaller, simpler configuration,
	// so I expect it to be used more frequently than mainnet.
	genesisDump := core.DefaultConfigTestnet.Genesis
	netId := 2
...
// miner.
func startNode(ctx *cli.Context, stack *node.Node) *ngin.Ngin {
	// Start up the node itself
	StartNode(stack)

	// Unlock any account specifically requested
	var ng *ngin.Ngin
	if err := stack.Service(&ng); err != nil {
		glog.Fatal("ngin service not running: ", err)
	}
...
func version(ctx *cli.Context) error {
	...
	fmt.Println("Protocol Versions:", ngin.ProtocolVersions)
