...
type announce struct {
	...
	fetchHeader headerRequesterFn // [ngin/62] Fetcher function to retrieve the header of an announced block
	fetchBodies bodyRequesterFn   // [ngin/62] Fetcher function to retrieve the body of an announced block
}
...
type Fetcher struct {
	...
	fetchingHook       func([]common.Hash)     // Method to call upon starting a block (ngin/61) or header (ngin/62) fetch
	completingHook     func([]common.Hash)     // Method to call upon starting a block body fetch (ngin/62)
	importedHook       func(*types.Block)      // Method to call upon successful block import (both ngin/61 and ngin/62)
}
...
func (f *Fetcher) FilterHeaders(peer string, headers []*types.Header, time time.Time) []*types.Header {
	glog.V(logger.Detail).Infof("[ngin/62] filtering %d headers", len(headers))
...
func (f *Fetcher) FilterBodies(peer string, transactions [][]*types.Transaction, uncles [][]*types.Header, time time.Time) ([][]*types.Transaction, [][]*types.Header) {
	glog.V(logger.Detail).Infof("[ngin/62] filtering %d:%d bodies", len(transactions), len(uncles))
...
func (f *Fetcher) loop() {
	...
	for {
		...
		case notification := <-f.notify:
			...
			if notification.number > 0 {
				if dist := int64(notification.number) - int64(f.chainHeight()); dist < -maxUncleDist || dist > maxQueueDist {
					...
					glog.V(logger.Debug).Infof("[ngin/62] Peer %s: discarded announcement #%d [%s], distance %d", notification.origin, notification.number, notification.hash.Hex(), dist)
		...
		case <-fetchTimer.C:
			...
			// Send out all block (ngin/61) or header (ngin/62) requests
			for peer, hashes := range request {
				if glog.V(logger.Detail) && len(hashes) > 0 {
					...
					glog.V(logger.Detail).Infof("[ngin/62] Peer %s: fetching headers %s", peer, list)
				}
		...
		case <-completeTimer.C:
			...
			for peer, hashes := range request {
				if glog.V(logger.Detail) && len(hashes) > 0 {
					...
					glog.V(logger.Detail).Infof("[ngin/62] Peer %s: fetching bodies %s", peer, list)
				}
				// Create a closure of the fetch and schedule in on a new thread
			...
			for _, header := range task.headers {
				...
				if cond {
					...
					if header.Number.Uint64() != announce.number {
						glog.V(logger.Detail).Infof("[ngin/62] Peer %s: invalid block number for [%s]: announced %d, provided %d", announce.origin, header.Hash().Hex(), announce.number, header.Number.Uint64())
					...
					if f.getBlock(hash) == nil {
						...
						if blockEmpty {
							glog.V(logger.Detail).Infof("[ngin/62] Peer %s: block #%d [%s] empty, skipping body retrieval", announce.origin, header.Number.Uint64(), header.Hash().Hex())
						...
						glog.V(logger.Detail).Infof("[ngin/62] Peer %s: block #%d [%s] already imported, discarding header", announce.origin, header.Number.Uint64(), header.Hash().Hex())
						f.forgetHash(hash)
					}
				} else {
