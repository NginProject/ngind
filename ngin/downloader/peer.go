...
// Block header and body fetchers belonging to ngin/62 and above
...
type peer struct {
	...
	getRelHeaders  relativeHeaderFetcherFn // [ngin/62] Method to retrieve a batch of headers from an origin hash
	getAbsHeaders  absoluteHeaderFetcherFn // [ngin/62] Method to retrieve a batch of headers from an absolute position
	getBlockBodies blockBodyFetcherFn      // [ngin/62] Method to retrieve a batch of block bodies

	getReceipts receiptFetcherFn // [ngin/63] Method to retrieve a batch of block transaction receipts
	getNodeData stateFetcherFn   // [ngin/63] Method to retrieve a batch of state trie data
...
func (p *peer) FetchReceipts(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("body fetch [ngin/63+] requested on ngin/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peer) FetchNodeData(hashes []common.Hash) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("node data fetch [ngin/63+] requested on ngin/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peer) String() string {
	...
	return fmt.Sprintf("peer:%s@[%s] ngin/%d", p.id, p.name, p.version)
	//	strings.Join([]string{
