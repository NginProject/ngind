...
var (
	...
	maxQueuedHeaders  = 32 * 1024 // [ngin/62] Maximum number of headers to queue for import (DOS protection)
...
func New(mode SyncMode, stateDb ngindb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
...
func (d *Downloader) syncWithPeer(p *peer, hash common.Hash, td *big.Int) (err error) {
	...
	glog.V(logger.Debug).Infof("Synchronising with the network using: %s [ngin/%d]", p.id, p.version)
	if logger.MlogEnabled() {
	...
	defer func(start time.Time) {
		...
		glog.V(logger.Debug).Warnf("Synchronisation with [%v][ngin/%d] terminated after %v", p, p.version, elapsed)
		if logger.MlogEnabled() {
