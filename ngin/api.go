...
func stateAndBlockByNumber(m *miner.Miner, bc *core.BlockChain, blockNr rpc.BlockNumber, chainDb ngindb.Database) (*state.StateDB, *types.Block, error) {
...
// Sign calculates an Ngin ECDSA signature for:
...
// PublicBlockChainAPI provides an API to access the Ngin blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	config                  *core.ChainConfig
	bc                      *core.BlockChain
	chainDb                 ngindb.Database
	indexesDb               ngindb.Database
...
func getTransaction(chainDb ngindb.Database, txPool *core.TxPool, txHash common.Hash) (*types.Transaction, bool, error) {
...
func getTransactionBlockData(chainDb ngindb.Database, txHash common.Hash) (common.Hash, uint64, uint64, error) {
	var txBlock struct {
...
func (s *PrivateAccountAPI) EcRecover(data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Ngin signature (V is not 27 or 28)")
	}
...
func (api *PublicNgindAPI) BuildATXI(start, stop, step rpc.BlockNumber) (bool, error) {
	...
	progress, err := api.ngin.BlockChain().GetATXIBuildProgress()
	if err != nil {
		return false, err
	}
	...
	go core.BuildAddrTxIndex(api.ngin.BlockChain(), api.ngin.ChainDb(), atxi.Db, convert(start), convert(stop), convert(step))
