...
// NewProtocolManager returns a new ethereum sub protocol manager. The Ngin sub protocol manages peers capable
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	// Unregister the peer from the downloader and Ngin peer set
...
func (pm *ProtocolManager) Stop() {
	...
	glog.V(logger.Info).Infoln("Ngin protocol handler stopped")
}
...
// handle is the callback invoked to manage the life cycle of an ngin peer. When
...
// EthNodeInfo represents a short summary of the Ngin sub-protocol metadata known
// about the host peer.
type EthNodeInfo struct {
	Network    int         `json:"network"`    // Ngin network ID (1=Mainnet, 2=Testnet)
